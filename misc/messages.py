WELCOME = """ВНИМАНИЕ!!! 

VPN сервисы типа Wireguard в данный момент подвергаются блокировкам со стороны РКН.
Обязательно используйте пробный период для проверки работы VPN в вашем регионе.
Работа сервиса в вашем регионе не гарантирована.

Привет!

Этот бот предоставит Вам быстрый и безопасный доступ к сети Интернет \
при помощи VPN.

Перейди в *Главное меню* чтобы управлять ботом.
"""

MAIN_MENU_CREATED = """Уважаемые пользователи!
К сожалению бот вынужден прекратить работу. Сервера будут доступны еще месяц.
По его истечению они будут отключены. По вопросам обращайтесь @endurancevpnsupport
Спасибо за использование!
"""

MAIN_MENU = """ВНИМАНИЕ!!!

VPN сервисы типа Wireguard в данный момент подвергаются блокировкам со стороны РКН.
Работа сервиса в вашем регионе не гарантирована.

Добро пожаловать в *Главное меню*!

Здесь ты можешь управлять своей подпиской, получить настройки для VPN \
или инструкцию по установке.

Выбери команду из списка ниже:                                
"""

MAIN_MENU_EXPIRED = """ВНИМАНИЕ!!!

VPN сервисы типа Wireguard в данный момент подвергаются блокировкам со стороны РКН.
Работа сервиса в вашем регионе не гарантирована.

Добро пожаловать в *Главное меню*!

К сожалению, твоя подписка закончилась \U0001f614 

Чтобы продолжить пользоваться VPN продли подписку.
"""

BLOCK_TRIAL = "Вы не можете активировать пробный период!"

SUCCESS_TRIAL = """Благодарю за активацию пробного периода!

Пробный период заканчивается {date},
"""

MY_SUB = """ \U00002b50 *Моя подписка*

Ваш ID: *{id}*
Статус вашей подписки: *{status}*
Дата окончания: *{date}*

Выберите команду из списка ниже или нажмите "Menu" для того, чтобы увидеть весь список команд
"""

SUB_PLANS = """Приобретая Endurance VPN не забудьте ознакомьться с [Условиями использования](https://telegra.ph/Usloviya-ispolzovaniya-Endurance-VPN-04-14) \U0001f9d0

Выберите длительность подписки:
"""

HAVE_PROMOCODE = """Не забудьте активировать промокод, если он у вас есть!

Найти его можно в меню "Моя подписка" \U000027a1 "Мой промокод"
"""

ALREADY_USE_PROMOCODE = """Вы уже использовали промокод!

Теперь поделитесь своим с друзьями!
"""

ENTER_PROMOCODE = """Введите промокод:"""

NO_PROMOCODE = """Такого промокода не существует!
"""

SUCCESS_PROMOCODE = """Промокод применен!

Ваша скидка будет применена при следующем продлении подписки.
"""

APPLY_PROMOCODE = """ Ваш промокод:

`{code}`

Поделитесь им с друзьями, чтобы получить скидку *100₽* на следующее продление за каждого приведенного друга!

Максимальная скидка составляет *300₽*
"""

DETAIL_ORDER = """Ваш заказ *№{id}*.

Сумма: *{amount}₽*
Количество дней: *{days}*
Статус: *{status}*
"""

MAX_ORDERS = """У вас максимальное количество заказов!
Удалите или оплатите имеющиеся заказы.
"""

ORDERS = """*Ваши заказы*

Неоплаченные заказы автоматически удаляются через 24 часа
"""

NO_ORDERS = """У вас нет созданных или не оплаченых заказов.
"""

SUCCESS_ORDER = """Ваш счет *№{id}* на сумму *{amount}₽* оплачен.

Статус вашей подписки: *{status}*
Дата окончания: *{expires_at}*
"""

DELETE_ORDER ="""Ваш счет {id} на сумму {amount}₽ удален"""

NOTIFY_DELETE_ORDER ="""Ваш счет {id} на сумму {amount}₽ в статусе {status} скоро будет удален."""

DISCOUNT_FOR_INVITING_USER = """По вашему промокоду была активирована подписка!

Теперь Ваша скидка увеличилась на *100₽*
"""

MAX_DISCOUNT_FOR_USER = """По вашему промокоду была активирована подписка!

У вас уже максимальная скидка, поэтому вам не было начислено бонусов.

Продлите подписку с текущей скидкой, чтобы обнулить количество бонусов.
"""

GET_VPN_UNSUB = """У вас отсутствует *подписка*.

Чтобы получить настройки, оформите или продлите подписку.
"""

REQUEST_FOR_VPN = """Мы получили Ваш запрос на формирование настроек.

В течение 5 минут бот обработает Ваш запрос и пришлет настройки.

Ожидайте...
"""

WAIT_FOR_SETTINGS = """Ваши настройки формируются.

В течение 5 минут бот обработает Ваш запрос и пришлет настройки

Ожидайте...
"""

GET_SETTINGS_SUCCESS = """Вот ваш файл настроек для VPN \U0000261d

Чтобы воспользоваться им, прочитайте инструкцию в 
"Главном меню" или запросите их у бота командой /instruction
"""

GET_SETTING_ERROR = """К сожалению, что-то пошло не так, обратитесь \
в техническую поддержку: @endurancevpnsupport
"""

EXPIRED_SUB = """Срок действия Вашей подписки истек. \U00002639

Чтобы продолжить пользоваться VPN продлите подписку.
"""

SUB_NOTIFICATE = """Срок действия Вашей подписки скоро закончится.

Дата окончания: *{expires_at}*
Чтобы продолжить пользоваться VPN продлите подписку.
"""

OUTDATED_SUB_NOTIFICATION = """Срок действия Вашей подписки давно истек.

Если вы не продлите подписку в течении *7* дней, ваш VPN будет удален.
"""

OUTDATED_SUB = """Срок действия Вашей подписки давно истек.

Ваш VPN будет удален. Для получения нового VPN продлите подписку.
"""

INSTRUCTION = """*Инструкция по настройке:*

1. Получите файл настроек для вашего VPN по команде /getvpn или кнопке "Получить VPN" в главном меню.
2. Скачайте файл настроек, который вам пришлет бот.
3. Скачать из [Google Play](https://play.google.com/store/apps/details?id=com.wireguard.android) \
или [AppStore](https://apps.apple.com/ru/app/wireguard/id1441195209) приложение Wireguard.
4. Откройте приложение и справа внизу нажать на "+"
5. Выберите "Импорт из файла или архива"
6. Выберите скачанный файл.
7. Передвиньте ползунок для активации приложения.
8. Используйте интернет как обычно.

Приобретая Endurance VPN не забудьте ознакомьться с [Условиями использования](https://telegra.ph/Usloviya-ispolzovaniya-Endurance-VPN-04-14)

Если интернет не доступен, попробовать переподключить приложение передвинув ползунок в положение выкл/вкл.\
В случае если это не помогло написать @endurancevpnsupport
"""

SUPPORT = """В случае возникновения каких-либо вопросов по работе бота или сотрудничеству, обращайтесь в \
аккаунт @endurancevpnsupport
"""

ADMIN_SYNC_TIMEOUT_ERROR = """Не обновилась конфигурация сервера {server}! Сервер не ответил!"""
ADMIN_SYNC_ERROR = """Не обновилась конфигурация сервера {server}! Пока не понятно почему..."""
ADMIN_CHECK_TIMEOUT_ERROR = """Не прошла проверка идентичности файлов конфига сервера {server}! Сервер не ответил!"""
ADMIN_CHECK_ERROR = """Не прошла проверка идентичности файлов конфига сервера {server}! Пока не понятно почему..."""


ADMIN_HELLO = """Привет, Админ! Выбери команду из списка:"""

ADMIN_ENTER_ORDER_ID = """Введите ID заказа..."""

ADMIN_CHECK_ORDER = """Заказ *№{id}* от *{date}*:

Статус: *{status}*
"""

ADMIN_ORDER_NOT_EXIST = """Такого заказа не существует!"""
ADMIN_INT_ERROR = """Тут должны быть цифры!"""
ADMIN_ENTER_USER_ID = """Введите ID пользователя..."""
ADMIN_ENTER_SUB_DAYS = """Введите количество дней подписки..."""
ADMIN_ENTER_DISCOUNT = """Введите ID пользователя и размер скидки через пробел..."""
ADMIN_USER_ACTIONS = """Что делаем с пользователем?"""
ADMIN_SUB_UPDATED = """Подписка пользователя ID {id} изменена!"""
ADMIN_VPN_RESETED = """VPN пользователя ID {id} сброшен!"""
ADMIN_DISCOUNT_UPDATED = """Для пользователя ID {id} установлена скидка {discount}!"""
ADMIN_START_REBUILD_SERVER_CONF = """Обновление кофигурации на серверах запущено..."""
ADMIN_FINISH_REBUILD_SERVER_CONF = """Обновление конфигурации успешно завершено!"""
ADMIN_ERROR_REBUILD_SERVER_CONF = """Ошибка при обновлении конфигурации, смотри логи!"""